...
public class AdminDashboardServiceImpl implements AdminDashboardService {
    ...
    public AdminDashboardStatsResponse getStats() {
        return new AdminDashboardStatsResponse(
                ...
                categoryRepository.countByIsPublicTrue(),
                questionRepository.countByQuestionStatus(QuestionStatus.PENDING),
    ...
    private List<RecentAttemptResponse> getCombinedRecentActivity() {
        ...
        attemptRepository.findTop10ByEndedAtIsNotNullOrderByStartedAtDesc().forEach(a ->
                activities.add(RecentAttemptResponse.builder()
                        .id(a.getId()).studentName(a.getQuizTaking().getLearner().getFullName())
                        .quizTitle(a.getQuizTaking().getQuiz().getTitle()).score(a.getResult())
                        .startedAt(a.getStartedAt()).type("QUIZ").build()));
        practiceRepository.findTop10ByIsCompletedTrueOrderByCreatedAtDesc().forEach(p -> {
            ...
            if (p.getTotalQuestions() != null && p.getTotalQuestions() > 0)
                score = BigDecimal.valueOf((p.getCorrectAnswers() * 10.0) / p.getTotalQuestions())
                        .setScale(1, java.math.RoundingMode.HALF_UP);
            activities.add(RecentAttemptResponse.builder()
                    .id(p.getId()).studentName(p.getUser().getFullName())
                    .quizTitle("Luyen tap: " + (p.getCategory() != null ? p.getCategory().getName() : "N/A"))
                    .score(score).startedAt(p.getCreatedAt()).type("PRACTICE").build());
        });
        ...
        return AdminDashboardDetailsResponse.builder()
                .recentUsers(userRepository.findTop5ByOrderByCreatedAtDesc().stream()
                        .map(u -> AdminDashboardDetailsResponse.DashboardUserResponse.builder()
                                .fullName(u.getFullName()).email(u.getEmail()).role(u.getRole().name())
                                .createdAt(u.getCreatedAt() != null ? u.getCreatedAt().format(fmt) : "N/A").build())
                ...
                .pendingModeration(questionRepository.findTop5ByQuestionStatusOrderByIdDesc(QuestionStatus.PENDING).stream()
                        .map(q -> AdminDashboardDetailsResponse.DashboardModerationResponse.builder()
                                .id(q.getId()).text(q.getText()).type("Cau hoi")
                                .creatorName(q.getCreator() != null ? q.getCreator().getFullName() : "N/A")
                                .categoryName(q.getCategory() != null ? q.getCategory().getName() : "N/A").build())
                        .collect(Collectors.toList()))
                .categories(categoryRepository.findTop5ByIsPublicTrueOrderByIdDesc().stream()
                        .map(c -> AdminDashboardDetailsResponse.DashboardCategoryResponse.builder()
                                ...
                                .questionCount(questionRepository.countByCategoryId(c.getId())).build())
                        .collect(Collectors.toList()))
                .build();
    }
}
