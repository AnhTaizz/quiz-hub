...
public class TeacherHomeServiceImpl implements TeacherHomeService {
    ...
    public TeacherHomeDashboardDTO getDashboardData(String email) {
        ...
        List<TeacherDashboardAssignmentDTO> assignments = quizAssigningRepository
                .findByClassroomCreatorId(teacher.getId()).stream()
                .map(qa -> {
        ...
        long liveQuizCount = assignments.stream().filter(a -> "LIVE".equals(a.getStatus())).count();
        return TeacherHomeDashboardDTO.builder()
                .classroomCount(classroomCount).questionCount(questionCount)
                .studentCount(studentCount).assignments(assignments)
                .liveQuizCount(liveQuizCount).currentUser(teacher).build();
    }
}
